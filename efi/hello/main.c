...
EFI_STATUS run(EFI_HANDLE ImageHandle) {
	...
	if(img == NULL) abort("could not get LOADED_IMAGE");

	// get the current volume
	EFI_FILE_HANDLE vol = LibOpenRoot(img->DeviceHandle);
	if(vol == NULL) abort("could not get volume FILE_HANDLE");

	// open the data file
	EFI_FILE_HANDLE file = NULL;
	vol->Open(vol, &file, L"data", EFI_FILE_MODE_READ, 0);
	if(file == NULL) abort("could not open file");

	EFI_FILE_INFO* info = LibFileInfo(file);
	if(info == NULL) abort("could not get file info");

	UINT64 size = info->FileSize;
	Print(L"size: %d bytes\n", size);

	UINT8* buf = AllocatePool(size + 2);
	if(buf == NULL) abort("could not allocate buffer");
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SystemTable) {
	...
	EFI_STATUS ret = run(ImageHandle);
